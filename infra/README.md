...
> **Edit `sre-agent.parameters.json` first.** It ships with placeholder values
> (e.g. `"<your-subscription-id>"`); the deployment will fail validation until you
> replace them with real subscription IDs, resource group names, and target scopes.
>
> **Supported regions.** `minimal-sre-agent.bicep` constrains `location` to the regions
> where Azure SRE Agent is currently available: `swedencentral`, `uksouth`, `eastus2`
> (default), and `australiaeast`. Other regions are rejected at template validation —
> check [sre.azure.com/docs](https://sre.azure.com/docs/) for the latest region list.
