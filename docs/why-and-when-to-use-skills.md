...
> **Example from this repo:** The [`high-cpu-vm-troubleshooting`](../plugins/vm-sre-skills/skills/high-cpu-vm-troubleshooting/) skill defines a fixed sequence: check Azure Monitor metrics → verify VM agent health → run OS-level diagnostics (both Linux and Windows) → correlate with Activity Log → produce structured report. Every investigation follows this path.
...
> **Example from this repo:** The [`disk-expansion`](../plugins/vm-sre-skills/skills/disk-expansion/) skill knows that your team requires a minimum of 20% free space after expansion, that Premium SSD v2 has different resizing rules than tier-based disks, and that MBR partitions can't exceed 2 TiB.
...
> **Example from this repo:** The [`disk-expansion`](../plugins/vm-sre-skills/skills/disk-expansion/) skill mandates: (1) always snapshot before changes, (2) get operator confirmation before deallocation, (3) never shrink a disk, (4) verify new size in both Azure and OS after expansion. These aren't suggestions — they're steps the agent must follow.
