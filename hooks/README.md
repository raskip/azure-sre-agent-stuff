...
5. [Hook examples included](#hook-examples-included)
6. [Practical examples](#practical-examples)
7. [Achieving true prevention](#achieving-true-prevention)
8. [How to configure hooks](#how-to-configure-hooks)
9. [Hook stacking — multiple hooks, same event](#hook-stacking--multiple-hooks-same-event)
10. [Prompt vs Command — when to use which](#prompt-vs-command--when-to-use-which)
...
### Hook events supported today (and what's *not* yet supported)

Per Microsoft's current documentation ([Microsoft Learn](https://learn.microsoft.com/azure/sre-agent/agent-hooks) and [sre.azure.com](https://sre.azure.com/docs/capabilities/agent-hooks)), Azure SRE Agent currently supports **two** hook events:

| Event | Triggers when | What you can do |
|---|---|---|
| **Stop** | The agent is about to return its final response | Validate completeness, force the agent to keep working |
| **PostToolUse** | A tool call finishes successfully | Audit it, block the result, inject extra context |

Hooks that exist in other agent runtimes — `PreToolUse`, `UserPromptSubmit`, `SessionStart`, `Notification` — are **not** yet shipped for Azure SRE Agent. If you see references to them in third-party material, they don't apply here. We'll add them to this example catalogue the moment Microsoft ships them.
...
| File | Event | Type | Matcher | failMode | Activation | Demo narrative |
|---|---|---|---|---|---|---|
| allowlist-remediation.yaml | PostToolUse | command | Bash\|… | block | On Demand | Remediation demos — only pre-approved commands run |
| audit-all-tool-usage.yaml | PostToolUse | command | `*` | allow | On Demand | Compliance / audit angle for any demo |
| block-dangerous-commands.yaml | PostToolUse | command | Bash\|… | block | Always | Safety baseline for every demo |
| block-vm-deletion.yaml | PostToolUse | command | Bash\|… | block | Always | Shared / demo environments — protects against `az vm delete` |
| enforce-structured-response.yaml | Stop | command | — | allow | Always | Diagnostic demos need consistent output format |
| require-evidence-in-diagnostics.yaml | Stop | command | — | allow | Always | "Show me the metric, not vibes" |
| require-summary-section.yaml | Stop | prompt | — | allow | Always | Always end with a Summary heading |
| restrict-to-readonly.yaml | PostToolUse | command | Bash\|… | block | On Demand | Diagnostic-only demos, blocks all writes |
| **prompt-completeness-check.yaml** | Stop | **prompt** | — | allow | Always | LLM grades response — root cause + remediation + verification all required |
| **prompt-evidence-quality.yaml** | Stop | **prompt** | — | allow | Always | LLM checks evidence is concrete, not vague |
| **prompt-blameless-language.yaml** | Stop | **prompt** | — | allow | On Demand | Post-mortem / customer-facing summaries — no individual blame |
| **kusto-only-audit.yaml** | PostToolUse | command | `ExecuteKustoQuery` | allow | Always | Audits every KQL the agent runs |
| **python-output-truncate.yaml** | PostToolUse | command | `ExecutePythonCode` | allow | Always | Caps long Python output to keep agent context clean |
| **mcp-tool-budget.yaml** | PostToolUse | command | `mcp_.*` | allow | On Demand | Limits MCP tool calls per thread (cost / rate limits) |
| **stack-audit-plus-policy.yaml** | PostToolUse | command | `*` and Bash\|… | both | Always | Two hooks on one event — audit + safety |
| **stop-strict-no-rejection-limit.yaml** | Stop | command | — | allow | On Demand | `maxRejections: 25` — strict format training |
| **exit-code-only-marker.yaml** | Stop | command | — | allow | Always | Simplest possible hook — bash one-liner using exit codes |
| **skill-aware-evidence.yaml** | Stop | **prompt** | — | allow | Always | Different acceptance bar per active skill (App Insights vs VM vs disk) |

Bold rows were added as part of the Tier 1 marketplace + knowledge demo set. Total: 18 examples.
    ...
    - type: command
      ...
      script: |
        ...
        approved = [
            ...
            # VM deallocate / start — required for OS-disk expansion and resize flows
            (r'az\s+vm\s+deallocate\b', 'VM deallocate (resize / OS-disk expansion)'),
            (r'az\s+vm\s+start\b', 'VM start (after resize / expansion)'),
            # Disk expand
...
## Hook stacking — multiple hooks, same event

When you attach more than one hook to the same event (**Stop** or **PostToolUse**), **all of them run** in declaration order. Each can independently allow or block; the strictest decision wins. `additionalContext` injections from multiple hooks chain, so the agent sees the final combined guidance from the hook sequence.

See [`examples/stack-audit-plus-policy.yaml`](examples/stack-audit-plus-policy.yaml) for the canonical example:

```yaml
hooks:
  PostToolUse:
    # Hook 1 — audit every tool call, never blocks
    - type: command
      matcher: "*"
      failMode: allow
      script: |
        # ... emit "[AUDIT-1] tool=<name>"

    # Hook 2 — only fires on shell tools, blocks on dangerous patterns
    - type: command
      matcher: "Bash|ExecuteShellCommand"
      failMode: block
      script: |
        # ... block rm -rf, sudo, chmod 777, raw block-device overwrite
```

Both hooks fire on every PostToolUse. Hook 1 always allows. Hook 2 may block. The agent only proceeds if neither hook rejects.

---

## Prompt vs Command — when to use which

```
┌── Do you need NUANCED judgement? (Is this response complete? Is this language blameless?)
│   ├── YES  → prompt hook (LLM evaluates)
│   └── NO   → ↓
│
└── Is the check DETERMINISTIC? (Block "rm -rf", require a marker, count calls)
    └── YES  → command hook (bash/python script)
```
...
|---|---|---|
| "Does the response cover all three: root cause, fix, verification?" | prompt | `prompt-completeness-check.yaml` |
| "Block `rm -rf` in any shell tool" | command | `block-dangerous-commands.yaml` |
| "Is the evidence specific or vague?" | prompt | `prompt-evidence-quality.yaml` |
| "Truncate Python output over 4500 chars" | command | `python-output-truncate.yaml` |
| "Apply a different evidence bar per active skill" | prompt (with transcript-reading) | `skill-aware-evidence.yaml` |

Prompt hooks cost time and money because each check is an LLM call. Command hooks are essentially free. Default to command; reach for prompt when you genuinely need subjective evaluation.
...
| Resource | Link |
|----------|------|
| Why and when to use hooks (this repo) | [`../docs/why-and-when-to-use-hooks.md`](../docs/why-and-when-to-use-hooks.md) |
| Creating skills and hooks with Copilot | [`../docs/creating-skills-and-hooks-with-copilot.md`](../docs/creating-skills-and-hooks-with-copilot.md) |
| Install everything via marketplace | [`../.github/plugin/marketplace.json`](../.github/plugin/marketplace.json) |
| Agent Hooks — sre.azure.com | https://sre.azure.com/docs/capabilities/agent-hooks |
| Agent Hooks — Microsoft Learn | https://learn.microsoft.com/azure/sre-agent/agent-hooks |
| Tutorial: configure hooks via API | https://learn.microsoft.com/azure/sre-agent/tutorial-agent-hooks |
| Tutorial: create hooks in the portal | https://learn.microsoft.com/azure/sre-agent/create-manage-hooks-ui |
| Run modes | https://sre.azure.com/docs/concepts/run-modes |
| Skills documentation | https://sre.azure.com/docs/concepts/skills |
