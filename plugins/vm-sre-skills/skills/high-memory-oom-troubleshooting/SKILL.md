...
examples:
  - "Investigate memory pressure and OOM kills on vm-app-001"
  - "Find VMs in rg-prod with persistent swap pressure"
  - "Explain whether vm-db-01 needs more memory or tuning"
  - "Report top memory consumers and cgroup limits on vm-api-01"
---
...
> **Note:** Windows services may run under Job Objects or inside containers. Use Job Object status for host-level constraints, and `docker stats` or `crictl stats` when the workload is containerised.
...
- **Linux OOM killer logs may be rotated** — if `dmesg` shows no OOM events but the issue is suspected, check `/var/log/syslog` or `/var/log/messages` for older entries.
- **Windows OOM-equivalent signals are different** — focus on pagefile usage, Event IDs 2019/2020, and Windows Error Reporting crash activity rather than looking for a Linux-style OOM killer.
