...
examples:
  - "Investigate why SSH to vm-app-001 is timing out"
  - "Find VMs in rg-prod blocked by NSGs or routes"
  - "Explain whether vm-prod-web-01 is failing at network or OS"
  - "Check RDP reachability, firewall, and service on vm-win-01"
---
...
6. **OS-level firewall blocking** — iptables/ufw/firewalld (Linux) or Windows Firewall is dropping traffic on the target port.
   → Add a firewall rule to allow the port, or disable the blocking rule.

7. **Boot failure** — Boot diagnostics show kernel panic, GRUB issues, or the VM failed to boot.
   → Use serial console to recover, or repair the OS disk by attaching it to a rescue VM.

8. **No public IP / wrong IP** — The VM has no public IP assigned, or the client is connecting to a stale IP.
