...
examples:
  - "Investigate why nginx keeps restarting on vm-prod-web-01"
  - "Find crash-looping services across VMs in rg-backend"
  - "Explain whether appsvc failures are config, dependency, or OOM"
  - "Report restart counts and recent errors for api-service on vm-app-001"
---
...
**Discover crash-looping services:**
```
az vm run-command invoke --resource-group {rg} --name {vm-name} --command-id RunPowerShellScript --scripts "Get-Service | Where-Object {$_.Status -eq 'Stopped' -and $_.StartType -ne 'Disabled'} | Select-Object Name, Status, StartType; Get-WinEvent -FilterHashtable @{LogName='System'; Id=7034,7031} -MaxEvents 20 | Format-Table TimeCreated, Id, ProviderName, Message -Wrap"
```
...
**Check restart count and timing:**
```
az vm run-command invoke --resource-group {rg} --name {vm-name} --command-id RunPowerShellScript --scripts "$events = Get-WinEvent -FilterHashtable @{LogName='System'; Id=7036} | Where-Object {$_.Message -match '{service-name}'}; $events | Select-Object -First 10 TimeCreated, Id, Message | Format-Table -Wrap; $events | Measure-Object"
```
...
```
az vm run-command invoke --resource-group {rg} --name {vm-name} --command-id RunPowerShellScript --scripts "Get-ChildItem 'C:\Windows\Minidump' -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 5 FullName, LastWriteTime; Get-ChildItem \"$env:LOCALAPPDATA\CrashDumps\" -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 5 FullName, LastWriteTime"
```
...
**Check for dependency services:**
```
az vm run-command invoke --resource-group {rg} --name {vm-name} --command-id RunPowerShellScript --scripts "(Get-Service -Name '{service-name}').ServicesDependedOn | Select-Object Name, Status; Get-Service -Name '{service-name}' -DependentServices | Select-Object Name, Status"
```
